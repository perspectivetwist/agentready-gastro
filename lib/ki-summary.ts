...
import { ActionItem } from './transformer'
...
export async function generateKiSummary(
  ...
  const actionsText = sanitizeInput(
    actions.map(a => `[${a.priority}] ${a.dimension}: ${a.action}`).join(' | ')
  )
  ...
  const message = await client.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `Restaurant: ${restaurantName} | Befunde: ${actionsText} | Antworte EXAKT so: { "zusammenfassung": "Abs.1: Was passiert wenn ein KI-Agent heute versucht für einen Gast einen Tisch zu reservieren, Öffnungszeiten abzurufen oder die Speisekarte zu lesen.\\n\\nAbs.2: Welche Buchungen und Gäste dadurch verloren gehen.\\n\\nAbs.3: Wie es bei einem agent-ready Restaurant aussieht.\\n\\nAbs.4: Was der Aktionsplan darunter konkret fixt. Keine Gedankenstriche." }`,
      },
    ],
  })
  ...
  try {
    ...
    // JSON.parse failed (likely literal newlines in string values)
    // Extract zusammenfassung value directly via regex
    const valueMatch = cleaned.match(/"zusammenfassung"\s*:\s*"([\s\S]*)"/)
    if (valueMatch) {
      return validateOutput({ zusammenfassung: valueMatch[1].replace(/\\n/g, '\n').replace(/\\"/g, '"') })
    }

    // Fallback: extract absatz_N values via regex
    const paragraphs: string[] = []
    for (let i = 1; i <= 6; i++) {
